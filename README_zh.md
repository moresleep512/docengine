...
- 有界、可续接的 Session 事件流，提供订阅总量预算与原子统计，精确报告慢消费者
  丢失量，发布保存/压缩/虚拟化进度与恢复 WAL 耐久性跃迁，并提供并发关闭屏障；
...
- recovery journal 默认 4 GiB 硬上限、精确增长统计，以及显式启用且失败后退避的自动
  保存检查点；
- 绑定 revision、UTF-8 安全的逻辑 Page 虚拟化，带严格 Page/窗口及全局活跃字节预算、
  有界 LRU 缓存、可取消 Provider 和可超时清理；
- generation 原子发布的格式中立 Fragment、明确的索引水位、定点 Measure 索引、
  byte/Fragment/Measure 三类锚点、非对称 overscan、巨型 Fragment continuation Page、
  单调进度，以及带 lineage 校验的当前 Session Snapshot 刷新。
...
undo 存储 256 MiB、保留 256 个事件、最多 128 个并发事件订阅、保留 256 个
ChangeMap、每批最多 65,536 个 Anchor、journal 默认硬上限 4 GiB，并且每秒同步一次。
自动 journal checkpoint 默认关闭；
...
`AfterSequence` 可续接消费，并在历史已淘汰时报告缺口。Session 还独立限制订阅总数；
`EventStats` 报告保留历史、存活订阅、丢弃投递、历史缺口、关闭及计数器耗尽状态。
目前发布打开、恢复、已提交的 Apply/Undo/Redo 变更、保存/压缩开始、进度、完成或失败、
恢复 WAL Sync 失败/恢复和关闭事件。进度通过 operation ID 关联，并能区分提交前失败与
已经提交的清理/耐久性错误。多个并发 `Close` 调用者等待同一个资源退役屏障并得到
相同结果。
...
Snapshot lease，直到调用方关闭。`MaximumInflightBytes` 对并发读取预留的 payload 总量
实施独立硬上限，不再用任务数乘单窗口预算近似。`Refresh` 向 Provider 提供可取消的
单调进度回报器，并且只发布完整 generation；`CloseContext` 会取消已接纳的 Provider/
读取任务，调用方超时也不会遗弃后台清理，所有关闭调用共享同一个 Source 释放结果。
`Session.RefreshVirtualPager` 校验上一 Pager 的 lineage，继承其精确资源策略，在保留旧
Pager 可用性的同时从当前不可变 Snapshot 建立新 Pager；进度同时进入有界 Session 事件流。

`Session.Compact` 只合并同 Source 且物理连续的 Piece，并把 undo store 重写为仍被历史
引用的数据。重写会先校验引用并以 64 KiB 取消检查点复制到候选 store，只有完整成功后
才切换活动文件、重映射历史并执行不会失败的 Piece 结构压缩。提交前失败不改变活动
store、历史或 Tree；旧文件清理失败会明确报告 `Committed=true`。
`CompactOptions.CheckpointJournal` 会先明确保存选定 revision，再重基追加式 journal；
未提交 WAL 绝不会被原地改写，因为那会破坏崩溃原子性或 revision 身份。压缩事件用
`CompactionProgress.OperationID` 关联，报告精确唯一存活字节并以 4 MiB 为进度粒度。
已经发出的不可变 Snapshot 仍可继续读取。
...
当前每个 package 都强制 100% 语句覆盖率，并包含二十七个 Go fuzz target：
...
- 逻辑 Page 分区、UTF-8 重组、Fragment 窗口不变量、独立 byte/Fragment/Measure 窗口
  参考模型，以及 Pager generation 和 Refresh/progress/close 状态机。
...
v0.5.6 Event/Compaction 套件在 Windows 本机与 WSL 2 Debian 均保持六包 100%
statement coverage；Linux 全仓三轮 shuffled race 无排除通过。Windows 新增的事件/
压缩/undo 路径通过 100 轮普通重复和 10 轮 race 重复；Windows 全仓 race 仍会复现
已经单独登记、按计划延后处理的 journal-sync 事件超时与打开句柄清理失败。事件与
undo 重写状态机 fuzz 在每端各运行 30 秒。零订阅和 128 个积压订阅的事件发布均为
零分配，约为 29 ns 与 12.5–13.2 µs；4 MiB 存活 undo 重写在 Windows 约
3.43–3.51 ms、Linux 约 3.41–3.72 ms。

v0.5.7 Virtual 套件在 Windows 本机与 WSL 2 Debian 继续保持六包 100% statement
coverage；Linux 全仓三轮 shuffled race 无排除通过。受影响的 Virtual/Session 路径在
Windows 通过 100 轮普通重复和 10 轮 race 重复，在 Linux 通过 10 轮 race 重复。六个
Virtual fuzz 每个平台均运行 30 秒；两个新增参考/状态机目标在 Windows 约执行 810 万
输入、Linux 约执行 500 万输入。缓存命中的 64 KiB Window 在 Windows 约
12.5–17.0 µs、Linux 约 21.3–22.5 µs，约 66 KiB/9 alloc；Refresh progress 在两端约
1.9–2.3 µs、1,288 B/22 alloc，增加 Observer 不增加分配。已单独登记的 Windows
journal-sync 事件/句柄间歇失败按计划没有混入此 tag。

v0.5.8 Windows CI 补丁让 journal-sync 事件测试不再依赖机器速度。较慢 runner 上，
一个已经捕获旧 journal 的后台 Sync 可以在 Save 进行中合法完成，事件顺序会是
`SaveStarted → JournalSyncRestored → SaveProgress → Saved`；旧测试却错误要求恢复一定
由 Save 自身触发。现在需要证明该窄语义的测试使用一小时后台周期，另一个阻塞写入的
确定性测试专门覆盖合法交错。每个测试在取得 Session/Subscription 后立即注册清理，
断言失败也不会遗留 Windows undo-store 句柄。Windows 与 WSL 均通过全仓三轮普通/
三轮 race、六包 100% coverage，以及五个 journal-sync 测试的 100 轮 race 重复；CI
另外在 Windows 执行 50 轮压力回归。
...
- 文件 watcher 候选变化尚未实现；保存、压缩、恢复 WAL 与 Virtual 发布/刷新状态跃迁
  已经进入事件流；
- journal 压缩必须显式建立保存检查点；搜索索引、搜索索引压缩和 Composition 尚未实现；
- Fragment 元数据和逻辑 Page 表受 Page、Fragment、key、任务、缓存、窗口策略及全局
  活跃字节配置限制。宿主可以继续持有已返回副本，因此还必须把这些结果计入自己的
  内存预算；
- 公开 API 和磁盘格式在 1.0 前仍不稳定。

## 下一步

v0.5.x 模块维护线与 Windows CI 收尾已经完成。下一阶段 v0.6 开始格式中立搜索。
完整完成度评估、目标架构和后续路线见 [develop.md](develop.md)。
