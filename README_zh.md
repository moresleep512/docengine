...
当前每个 package 都强制 100% 语句覆盖率，并包含二十个 Go fuzz target：

- Piece Tree 参考模型、并发 Snapshot/edit 与压缩/Snapshot 保留；
...
v0.5 实现已在 Windows 本机验证：六个 package 全部达到 100% statement coverage，
全仓三轮 shuffle race 通过，四个虚拟化 fuzz target 各运行 10 秒通过；六个 Linux
测试二进制也全部交叉编译成功。当前工作站没有安装可用的 WSL 发行版，因此本轮本地
结果不冒充原生 Linux 执行，Linux 运行验证留给 Ubuntu CI。
...
- Fragment 元数据和逻辑 Page 表受 Page、Fragment、key、任务及缓存配置限制；缓存预算
  不包含活跃任务持有的瞬时副本，读取峰值还受
  `MaximumTasks × MaximumPageBytes` 约束；
