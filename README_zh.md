...
目录时才删除，脏恢复 journal 和宿主的未知文件始终保留。`Session.Config` 可读取最终
解析后的策略。
...
九个 Session/event/change-history/coordinate fuzz target 在两端分别运行 10 秒并通过。
...
下一块 v0.4 工作是失效 Session 目录回收、剩余持久化/进度事件、通用保留区间与
annotation 变换，以及第一版压缩策略。之后在现有地基上实现格式中立的逻辑
