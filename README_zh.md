...
- 有界、可续接的 Session 事件流，提供订阅总量预算与原子统计，精确报告慢消费者
  丢失量，发布保存/压缩进度与恢复 WAL 耐久性跃迁，并提供并发关闭屏障；
...
- 文件 watcher 候选变化和 Virtual 刷新/进度事件尚未实现；保存、压缩及恢复 WAL
  状态跃迁已经发布；
...
v0.5.x 维护线下一步补齐 Virtual 的刷新、进度、身份和性能边界；所有现有模块收口后，
再单独修复已知 Windows journal durability CI 竞态并发布一个小版本。之后 v0.6 才开始
格式中立搜索。完整完成度评估、目标架构和后续路线见
