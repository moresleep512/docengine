...
已完成自动 checkpoint 数。显式目录默认是 shared；省略 SessionDir 时会创建唯一的 owned
...
当前每个 package 都强制 100% 语句覆盖率，并包含二十三个 Go fuzz target：

- Piece Tree 参考模型、并发 Snapshot/edit、压缩/Snapshot 保留与自动压缩策略；
- v2 文件头、操作 decoder、回放韧性和 journal 状态机；
- Session 状态机、并发保存/编辑、崩溃恢复、journal 配额原子性与 UTF-8 编辑边界；
- 可续接事件历史、订阅溢出和关闭状态机；
...
checkpoint 失败退避，以及带/不带并发编辑时分别在替换前后的真实子进程退出。事件测试
还覆盖精确丢失计数、
