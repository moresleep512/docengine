...
| Session 生命周期与公共 API | 约 95% | revision、undo/redo、并发保存、只读 fault、配置化限制、事件/进度流、订阅总量预算与统计、Context 取消、可超时关闭屏障、跨 generation Snapshot lease 硬预算与统计、有界 ChangeMap、持锁孤儿回收、原子可取消压缩 | watcher、全局资源指标、稳定错误/兼容承诺 |
...
| Page/Fragment 虚拟化 | 约 96% | UTF-8 逻辑 Page、FragmentProvider、索引水位、Measure、三类锚点、overscan、continuation、generation CAS、LRU/任务/活跃字节硬预算、Provider 取消、进度事件、lineage 与跨 revision 托管刷新 | 自动调度策略与长期性能/内存 soak |
...
综合看，距离“完备、可稳定嵌入的通用文档编排内核”约完成 75%，误差约
...
v0.5.6 Event/Compaction 套件在两端继续保持六包 100% statement coverage；Linux
全仓三轮 shuffle race 无排除通过。Windows 新增事件/压缩/undo 路径通过 100 轮普通
重复与 10 轮 race 重复；Windows 全仓 race 仍复现已单独登记并按计划后置的
journal-sync 事件超时和打开句柄清理失败。事件和 undo 重写状态机 fuzz 每端各运行
30 秒，事件发布与 4 MiB undo 重写基准也在两端执行。

v0.5.7 Virtual 套件在两端继续保持六包 100% statement coverage；Linux 全仓三轮
shuffle race 无排除通过。Virtual/Session 新增与受影响路径在 Windows 通过 100 轮普通
重复和 10 轮 race 重复，在 Linux 通过 10 轮 race 重复。六个 Virtual fuzz 每端各运行
30 秒，新增的三类窗口独立参考模型与 Refresh/progress/close 状态机在 Windows 约执行
810 万输入、Linux 约执行 500 万输入。缓存窗口和 Refresh progress 基准也在两端执行；
已知 Windows journal-sync 事件/句柄间歇失败保留给下一独立补丁版本。

v0.5.8 单独关闭该 Windows CI 问题：根因是事件测试把两个独立操作错误串成唯一顺序，
并且断言失败前未注册资源清理。后台 Sync 若已捕获旧 journal，可以在 Save 写入期间合法
发布恢复；确定性阻塞测试现在覆盖该交错，需要证明 Save 自身恢复的测试则隔离后台
ticker。Windows 与 WSL 均通过全仓三轮普通/三轮 race、六包 100% coverage 和五项
journal-sync 测试 100 轮 race 重复；Windows CI 另加 50 轮稳定性门禁。
...
v0.5.6 把“每个队列有界”提升为“整个 Session 有界”：默认最多 128 个并发订阅，硬上限
4,096，避免调用方持续创建订阅绕过单队列预算。`EventStats` 原子报告保留历史、存活
订阅、丢弃投递、历史缺口及关闭状态；丢失计数做饱和加法，订阅 ID 回绕跳过零和存活
冲突，事件 sequence 真正耗尽时关闭 hub 而不是复用因果 ID。压缩也发布关联 operation
ID 的开始、4 MiB 粒度进度、完成/失败事件，并用 `Committed` 区分候选丢弃和已切换后
旧文件清理失败。
...
不会按字节比例伪造布局。LRU 只计算驻留 payload；`MaximumInflightBytes` 独立约束所有
活跃 `ReadPage`/Window 预留字节，调用返回后由宿主持有的副本另计入宿主预算。
Provider 回调不持 Pager/Session 锁，可以读取 `Stats`，但不得同步调用同一 Pager 的
任务型 API 或 `Close`；慢结果不能覆盖新 generation。

v0.5.7 完成了剩余生命周期契约：每个 Refresh 都得到 call-scoped 单调进度回报器，
非法或倒退报告会粘滞失败并阻止最终发布；Publish/Refresh 用 operation ID 发布开始、
进展、完成或失败事件。`CloseContext` 取消所有已接纳任务，允许调用方超时退出但继续
唯一后台清理屏障。`Lineage` 防止 Session 把无关 Pager 当作自身历史；
`Session.RefreshVirtualPager` 继承旧 Pager 的精确资源策略，用当前 Snapshot 构建新
revision，而旧 Pager 继续可读。当前仅剩自动调度策略和长期性能/内存 soak，不影响无
Fragment 时的逻辑 Page fallback。
...
- Piece/undo 与保存检查点 journal 压缩首版已具备；journal 已有默认 4 GiB 硬上限和
  显式启用的自动 checkpoint 阈值；undo 重写在 64 KiB 块间检查取消，候选完整前不
  改变活动 store、history 或 Piece Tree，后续仍需索引 GC，且始终不能破坏存活
  Snapshot；
...
v0.4 的事件基础与 v0.5.6 的资源/压缩补强已经完成：

- 有界历史、每订阅者有界队列和 Session 订阅总量硬上限，慢消费者不阻塞 Session
  写锁；
- `AfterSequence` 续接、`FutureOnly`、历史缺口及实时溢出的精确 `Dropped`；
- `EventStats` 报告历史/订阅预算、实际丢弃与 sequence 耗尽；计数器不会静默回绕；
- 打开、恢复、Apply/Undo/Redo 提交、保存/压缩生命周期、恢复 WAL Sync 跃迁和最终
  关闭事件；失败事务与 no-op 不发布变更事件；
...
- 基于历史窗口批量变换 Anchor、AnchoredRange 与 opaque `Annotation[T]`，错误时不返回
  部分结果；
- Piece 只合并同 Source 连续范围；undo 在候选 store 完整后才切换并重映射历史，
  journal 仅通过显式保存检查点重基，均不破坏已发出的 Snapshot；
- 压缩取消和提交前 I/O 错误保持 store/history/Tree 三者不变；提交后清理失败带
  `Committed=true`，不会把已生效结果伪装成未执行。

后续高层仍需完成：

- watcher 候选变化、Virtual 自动调度，以及统一后台任务 generation/completeness 契约；
...
`v0.5.6` 补齐事件/压缩：新增默认 128、硬上限 4,096 的订阅总量预算，以及可在关闭后
读取的 `EventStats`；事件丢失和历史缺口计数饱和，订阅 ID 与 sequence 回绕有明确
行为。`Session.Compact` 先以 64 KiB 可取消块把唯一存活 undo 引用复制到候选文件，
只有候选完整后才切换、重映射历史并执行不会失败的 Piece 压缩。压缩结果和事件共享
operation ID、精确总字节和 4 MiB 进度；旧文件清理错误明确标为已提交。状态机 fuzz
分别覆盖事件队列与 undo 候选提交/取消，磁盘复制的异常 I/O 契约也通过最小内部接口
注入验证。

`v0.5.7` 补齐 Virtual：Pager 关闭会取消已接纳的读取/Provider Context，并通过可超时
共享屏障完成 Source 释放；Window 与 ReadPage 增加全局活跃字节硬预算。Provider 报告
单调进度，Publish/Refresh 以 operation ID 进入 Session 事件流。opaque lineage 和
`Session.RefreshVirtualPager` 把旧 Pager 策略安全带到当前 Snapshot，任何失败都关闭
新取得的 owned Source。独立窗口参考模型 fuzz 同时核对三类锚点的精确页选择、预算和
双向截断，第二个状态机 fuzz 核对 generation、进度阶段、粘滞报告错误和 Close。

至此 v0.5.x 模块维护线完成，不实现搜索、持久区间集合或 Composition。`v0.5.8` 又单独
关闭了 Windows CI 的 journal durability 事件/句柄清理失败；v0.5.x 完全收口，下一阶段
进入 v0.6。
