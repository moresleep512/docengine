...
| Session 生命周期与公共 API | 约 72% | revision、undo/redo、并发保存、只读 fault、Context 打开、配置化限制、目录生命周期、可续接事件流、关闭屏障 | 保存/fault/外部变化事件、崩溃孤儿回收、稳定错误/兼容承诺 |
| 坐标与 ChangeMap | 约 70% | revision 固定索引、有界查询、Anchor、ChangeMap 组合/反转、Session 提交映射、安全前缀增量复用 | 自动缓存、批量锚点、跨多个历史 revision 的保留策略、可证明的后缀复用 |
...
综合看，距离“完备、可稳定嵌入的通用文档编排内核”约完成 52%，误差约
±10%。最难的存储、原子事务、资源策略和单 revision 坐标地基已经存在；尚未完成的约 48%
主要是跨 revision 状态保留、虚拟化、搜索、组合、生命周期和生产级验证，而不是继续堆叠
编辑 API。
...
Session fuzz、一个事件状态机 fuzz 和三个 coordinate fuzz 在两端分别运行 10 秒并通过。
...
正确的后缀。自动缓存、跨多个历史 revision 的映射保留和可证明的后缀复用仍是后续边界。
...
完成；安全 checkpoint 前缀增量复用也已完成。剩余持久化/进度事件、崩溃孤儿回收、
跨 revision 映射保留、批量 Anchor 和第一版压缩仍待完成。
