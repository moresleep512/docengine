...
| 能力块 | 估算成熟度 | 已具备 | 主要缺口 |
| --- | ---: | --- | --- |
| Piece Tree、Snapshot、事务 | 约 93% | 磁盘 Source、结构共享、批次原子发布、快照隔离、同源连续 Piece 压缩 | 自动压缩阈值、系统化基准 |
| Recovery 与保存 | 约 84% | v2 原子批 journal、完整 SHA-256、CRC、尾部修复、跨平台原子替换、保存检查点重基 | 崩溃进程矩阵、自动 journal 阈值、更多文件系统验证 |
| Session 生命周期与公共 API | 约 86% | revision、undo/redo、并发保存、只读 fault、配置化限制、事件/进度流、关闭屏障、有界 ChangeMap、持锁孤儿回收、首版压缩 | watcher、后台任务治理、稳定错误/兼容承诺 |
| 坐标与 ChangeMap | 约 85% | revision 固定索引、有界查询、Anchor/Range/opaque Annotation、组合/反转、安全前缀复用、lineage、托管刷新、批量变换 | 自动缓存、持久区间集合、可证明的后缀复用 |
...
综合看，距离“完备、可稳定嵌入的通用文档编排内核”约完成 68%，误差约
±10%。存储、原子事务、资源策略、坐标变更与通用虚拟化地基已经存在；尚未完成的
约 32% 主要是搜索、组合、watcher/后台任务治理和生产级验证，而不是继续堆叠编辑 API。
...
v0.5 实现已在 Windows 本机完成六个 package 的 100% statement coverage、全仓三轮
shuffle race 和四个 virtual fuzz target 各 10 秒验证，六个 Linux 测试二进制交叉编译
通过。当前工作站没有可用 WSL 发行版，因此原生 Linux 本地执行不计入本轮已完成项，
由 Ubuntu CI 继续验证。
...
当前 Snapshot。批量 AnchoredRange 与 `Annotation[T]` 复用相同的端点 affinity 和工作
预算，payload 对内核保持 opaque。自动缓存所有权、持久区间集合和可证明的后缀复用
仍是后续边界。
...
`MaximumTasks × MaximumPageBytes` 另行约束。Provider 回调不持 Pager/Session 锁，慢
结果不能覆盖新 generation；并发 `Close` 等待同一任务屏障并共享 Source release 结果。
...
- 后台任务调度、Context 取消、优先级和跨 Page/搜索任务背压；Session 资源关闭屏障
  已具备，后台任务仍需纳入同一屏障；
- Piece/undo 与保存检查点 journal 压缩首版已具备；后续仍需自动阈值、索引 GC，且
  始终不能破坏存活 Snapshot；
