...
本文记录 v0.4 开发期间 Docengine 的边界、当前完成度、目标架构和到 1.0
...
| 能力块 | 估算成熟度 | 已具备 | 主要缺口 |
| --- | ---: | --- | --- |
| Piece Tree、Snapshot、事务 | 约 90% | 磁盘 Source、结构共享、批次原子发布、快照隔离 | Piece 压缩、系统化基准 |
| Recovery 与保存 | 约 80% | v2 原子批 journal、完整 SHA-256、CRC、尾部修复、跨平台原子替换 | 崩溃进程矩阵、journal 压缩、更多文件系统验证 |
| Session 生命周期与公共 API | 约 77% | revision、undo/redo、并发保存、只读 fault、配置化限制、目录生命周期、事件流、关闭屏障、有界 ChangeMap 历史 | 保存/fault/外部变化事件、崩溃孤儿回收、稳定错误/兼容承诺 |
| 坐标与 ChangeMap | 约 78% | revision 固定索引、有界查询、Anchor、ChangeMap 组合/反转、安全前缀复用、lineage、托管刷新、批量 Anchor | 自动缓存、通用区间、可证明的后缀复用 |
...
综合看，距离“完备、可稳定嵌入的通用文档编排内核”约完成 54%，误差约
±10%。最难的存储、原子事务、资源策略和坐标变更地基已经存在；尚未完成的约 46%
主要是通用区间、虚拟化、搜索、组合、生命周期和生产级验证，而不是继续堆叠
编辑 API。
...
五个 package 均为 100% statement coverage，三轮 shuffle race 通过，涉及的四个
Session fuzz、一个事件状态机 fuzz、一个 ChangeMap 历史状态机 fuzz 和三个 coordinate
fuzz 在两端分别运行 10 秒并通过。
...
当前首批实现已经完成 Snapshot revision 固定的流式索引、UTF-8 安全 checkpoint、
byte/line/rune 双向转换、顺序 ChangeMap、Anchor before/after 粘性、组合与反转，并让
Session 的编辑、undo、redo 返回对应映射。首版增量重建接收精确 ChangeMap 链，只复用
所有顺序编辑之前的稳定 checkpoint 前缀，并从最后一个安全 checkpoint 重扫新 Snapshot；
旧、新 Index 各自持有独立 Source lease。它刻意不平移无法证明 rune/line/column 状态仍
正确的后缀。Session 现以有界 ring 保留跨 revision 映射，区分历史淘汰和不可观察的批次
内部 revision；opaque lineage 防止外来 Index 被错误复用，托管刷新会原子取得映射链与
当前 Snapshot。自动缓存所有权和可证明的后缀复用仍是后续边界。

每次非空 Apply、Undo 和 Redo 提交现在还会发布携带同一不可变 ChangeMap 的
`SessionEvent`。消费者可用单调 sequence 续接事件；发现 `Dropped > 0` 时不能继续盲目
套用映射，必须从当前 Snapshot 重建派生状态。
...
- 后台任务调度、Context 取消、优先级和跨 Page/搜索任务背压；Session 资源关闭屏障
  已具备，后台任务仍需纳入同一屏障；
...
- 打开、恢复、Apply/Undo/Redo 提交和最终关闭事件；失败事务与 no-op 不发布事件；
- 最终关闭事件在队列溢出时仍可达，所有并发 `Close` 调用共享同一个资源退役屏障；
- 退订、发布与 Session 关闭可并发，通道不会发生重复关闭或 send-on-closed。

下一阶段仍需完成：

- 保存开始/已提交/耐久性未知、fault、外部变化和索引进度事件；
- 事件的稳定错误载荷与后台任务 generation/completeness 契约；
- 基于现有批量 Anchor 和历史窗口的通用范围/区间批量变换；
...
当前进度：坐标索引、Anchor、ChangeMap、Session 返回映射、配置化限制、sync 周期、
undo 临时文件隔离、shared/owned 目录清理、可续接 ChangeSet 事件流及并发关闭屏障已
完成；安全 checkpoint 前缀增量复用、lineage 校验、有界跨 revision 映射保留和批量
Anchor 也已完成。剩余持久化/进度事件、崩溃孤儿回收、通用区间层和第一版压缩仍待完成。
